...
if __name__ == "__main__":
    ...
    while True:
        ...
        if data == 'Hi':
            ...
            computer_id = get_computer_id(map_id_map_client_changes)
            key = get_client_id()
            map_id_map_client_changes[key] = {computer_id: None}
            ...
            if data[0] == 'n':
                if data[1:] not in map_id_map_client_changes.keys():
                    server.close()
                    break

                # rand a computer id
                computer_id = get_computer_id(map_id_map_client_changes)
                map_id_map_client_changes[data[1:]][computer_id] = None
            ...
            # existing computer
                ...
                if map_id_map_client_changes[data[1:]][computer_id] is not None:
                    client_socket.send('updates from another computer'.encode() + b'\n')
                    client_socket.send(str(len(map_id_map_client_changes[data[1:]][computer_id])).encode() + b'\n')
                    for event in map_id_map_client_changes[data[1:]][computer_id]:
                ...
                try:
                    receive_changes(client_socket, data[1:], size, map_id_map_client_changes, computer_id)
                except:
                    pass
